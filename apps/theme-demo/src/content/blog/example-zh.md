---
title: Markdown 格式展示
excerpt: 本文展示了各种 Markdown 格式，用于测试主题如何渲染不同的内容
pubDate: 2025-01-15
category: Demo
readingTime: 10 mins
...
本文展示了各种 Markdown 格式，用于测试主题如何渲染不同的内容。本文将尽可能包含更多的 Markdown 元素。

## 标题级别

这是一个 H2 标题，以下是不同标题级别的示例：
...
这是一段常规文本，包含**粗体文本**、*斜体文本*、***粗斜体文本***、~~删除线文本~~和`行内代码`。

你也可以在句子中使用`代码片段`，例如 `const example = 'hello'`。
...
这是一个指向 Astro 网站的[行内链接](https://astro.build)。

这是一个带标题的[链接](https://github.com "GitHub")。

这是一个指向归档页面的[相对路径链接](../archive)。
...
> 这是一个包含**粗体**、*斜体*和`代码`的引用块。它也可以包含[链接](https://example.com)。
>
> - 引用块中的列表项 1
> - 引用块中的列表项 2
>   1. 嵌套有序列表
...
如果你需要显示 Markdown 语法字符本身，可以使用反斜杠进行转义：
...
[^note]: 这是一个命名脚注的内容，可以包含**格式**和[链接](https://example.com)。
...
希望这些示例能帮助你了解主题如何渲染不同的 Markdown 内容！
